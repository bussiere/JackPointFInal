...
def index(request):
    ...
    variables = {'notifications':notifications,'messagerecus':messagerecus,'messageenvoyes':messageenvoyes,'profile':profile,'user':user}
    variables = dict(variables.items() + locals().items())
    return render_to_response('x.html',variables,RequestContext(request))
